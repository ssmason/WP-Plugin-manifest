 ...
 * Security helpers.
 *
 * Centralises nonce creation, verification, and capability checks so
 * all AJAX handlers and admin views consume a single, auditable surface.
 *
...
class Security {
	...
	/**
	 * Verifies a nonce from the AJAX request and dies on failure.
	 *
	...
	/**
	 * Returns the nonce field HTML for embedding in forms.
	 *
	 * @author Stephen Mason <steve@satori-digital.com>
	 * @since  1.0.0
	 * @param  string $action        Nonce action key (without prefix).
	 ...
	 */
	public static function nonce_field( string $action, bool $referer = true, bool $should_echo = false ): string {
		return (string) wp_nonce_field(
			self::NONCE_PREFIX . $action,
			'satori_manifest_nonce',
			$referer,
			$should_echo
		);
	}
	...
	/**
	 * Returns the nonce action string (with prefix) for use in inline scripts.
	 *
	 * @author Stephen Mason <steve@satori-digital.com>
	 * @since  1.0.0
	 * @param  string $action  Nonce action key (without prefix).
